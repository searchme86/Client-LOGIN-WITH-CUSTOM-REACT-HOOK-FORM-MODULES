...
|   상태   |       카테고리        |                                          과업                                          |
| :------: | :-------------------: | :------------------------------------------------------------------------------------: |
|   완료   |   타입스크립트 도입   |                       베이스 JS리액트를 TS리액트로 마이그레이션                        |
|          |     프로젝트 시작     |                             TS리액트로 프로젝트 구조 완성                              |
|          |        구글링         |                 재사용가능한 React Hook Form에 대한 코드 및 이론 학습                  |
|          |         구현          |    재사용가능한 React Hook Form 구현 (이미지 업로드 -> 인풋 폼 -> 초기화/등록 버튼)    |
|          |       리펙토링        |                이전 포트폴리오에서 로그인/등록 페이지에서 핵심코드 추출                |
|          |        구글링         |                        타입스크립트와 Context Api 타입이슈 해결                        |
|          |    구글링 및 구현     |                         Axios InterCeptor를 TS로 마이그레이션                          |
|          | 리펙토링 - 등록페이지 |                         등록(register) 폼 만들기 (디자인 포함)                         |
|          |        테스트         |       커스텀 React Hook Form에서 값 생성 및 초기화/등록 버튼 동작하는지 개발작업       |
|          |        테스트         |                         등록 폼에서 Submit한 값이 서버로 전달                          |
|          |         구현          |                         등록(register) 폼 만들기 (디자인 포함)                         |
...
| 진행예정 |         구현          |                   백엔드로 Refresh Token을 요청하는 React-Query 개발                   |
| 진행예정 |        테스트         | AuthMiddleWare & Protected Router 백엔드로 Refresh Token을 요청하는 React-Query 테스트 |
| 진행예정 |         구현          |       React-Query와 Axios InterCeptor / Refresh Token이 서로 적용되는 코드 구현        |
| 진행예정 |         배포          |                                       코드 배포                                        |
