...
- React Hook Form이 적용된, 추상성이 높은 폼 엘리먼트(form, input) 구현
  ...
  - 폼 엘리먼트(form,input,select 등)는 React Hook Form이 적용되어 있어, 유효성 관리 기능을 추가하지 않아도 됨
...
- 💪🏻 [이전 포트폴리오,fleamarketagora](https://github.com/searchme86/App_Final_Deployed '이전 포트폴리오,fleamarketagora')의 코드를 리펙토링하고 개선을 통해, 스스로 기술적 성장을 목표함
  ...
  - 💡 폼과 React Hook Form이 한 쌍으로 자주 사용되어, 함께 모듈화되는 방법이 없을까 생각이 들었음
    ...
    - #### 🙏 개선 희망사항
      |                                       AS-IS                                       |                                                                                       개선 이유                                                                                       |                                              TO-BE                                              |
      | :-------------------------------------------------------------------------------: | :-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------: |
      |                        Cannot read properties of undefined                        |                                 - 코드 규모가 확장 될 수록, <br/> 값이 undefined이거나 <br/> 추적하기 어려움 <br/> - 값의 형태/타입을 찾아내기 어려움                                 |                      타입스크립트 <br/> 적용을 통해 <br/> 코드 안정성 확보                      |
      ...
      |               npm'react-file-base64'을 통해 <br/> 이미지 업로드 함                |                                 폼 상태를 초기화(reset) 할 때, <br/> 모듈 'react-file-base64'에 <br/> 저장된 값만 남아있어 <br/> 이를 처리하기 어려움                                 |                              이미지 업로드 <br/> 과정 하나씩 구현                               |
      |               (Chakra UI) <br/> Chakra UI를 통한, 폼 컴포넌트 구현                |                                                                   ref로 input을 참조 되지 않는 <br/> 버그🐛를 경험                                                                    |                                         Chakra UI 제외                                          |
      |       (styled-components) <br/> styled-components를 통한 <br/> 스타일 구현        | 컴포넌트를 감싸거나, <br/> 자식으로 props 전달, <br/> 특별한 목적 없는 div, <br/> 혹은 일반 스타일까지도, <br/><br/> styled 컴포넌트로 만들어,<br/> 컴포넌트에 불필요한 코드가 낭비됨 |                              의미없는 <br/> 스타일 코드 생성 방지                               |
